...
def openAI(prompt):
    """Function that gets an AI response from OpenAI"""
...
def openAImage(prompt):
    """Function that gets an AI Image from OpenAI"""
    # Make the request to the OpenAI API
    resp = requests.post(
        ...
        try:
            ...
            if float(result['update_id']) > float(last_update):
                ...
                if not result['message']['from']['is_bot']:
                    ...
                    # Checking that user mentionned gpt username in message
                    if '/gpt' in result['message']['text']:
                        prompt = result['message']['text'].replace("/gpt", "")
                        print("got message will send to openai")
                        ...
                        print("answer from openai is: " + bot_response)
                        print(telegram_bot_sendtext(bot_response, chat_id, msg_id))
...
def main():
    """Function that runs the Chatbot function every 5 seconds"""
    timertime = 5
    Chatbot()
